...
def _run_discovery(run_id: str):
    """
    Discovery phase logic: preflight → screenshots of all discovered pages.
...
def execute_discovery(self, run_id: str):
    """Celery wrapper for discovery phase."""
    try:
        _run_discovery(run_id)
    except Exception as e:
        from apps.runs.models import Run
...
def _run_analysis(run_id: str):
    """
    Analysis phase logic: AI analysis → scoring → comparative analysis.
...
def execute_analysis(self, run_id: str):
    """Celery wrapper for analysis phase."""
    try:
